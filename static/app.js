...
function _buildOrbitalCache(modeName) {
    ...
    // In guided mode, let initial direction phi bias the gravity-turn program.
    // phi=90deg keeps nominal program unchanged; lower/higher phi tilts it.
    ...
    let vy = vInit * cosPhi;
    if (modeName === 'guided') {
        // Guided mode treats v0 as initial prograde/tangential insertion speed.
        // At launch point (x>0,y=0), local tangent is +Y.
        vx = 0;
        vy = vInit;
    }
    let hit = false;
    ...
    for (let i = 0; i <= n; i++) {
        ...
        if (modeName === 'powered' && tt <= tburn) {
            // Keep thrust pitch relative to local horizontal/radial basis.
            ax += athrust * (sinPhi * erx + cosPhi * etx);
            ay += athrust * (sinPhi * ery + cosPhi * ety);
        } else if (modeName === 'guided') {
            if (tt <= tburn1) {
                // Ascent + gravity turn: smoothly pitch from near-vertical to near-horizontal.
                ...
                // Circularization burn with simple guidance:
                // 1) trim tangential speed toward local circular speed at target radius
                // 2) damp radial velocity to reduce altitude oscillation
        ...
        // Symplectic Euler for stable orbital energy behavior at interactive step sizes.
        vx += ax * dt;
        vy += ay * dt;
        x += vx * dt;
        y += vy * dt;
